...
static void _cbBk(WM_MESSAGE * pMsg) {
  ...
  case WM_PAINT:
    ...
    {
      GUI_DrawBitmap(&bmbackground, 0,25);
      GUI_SetAlpha(0xA0);
      GUI_DrawBitmap(&bmSTM32_F4, (LCD_GetXSize() - bmSTM32_F4.XSize) / 2, ((LCD_GetYSize() - bmSTM32_F4.YSize) / 2) + 40); 
      GUI_SetAlpha(0x00);
    }
    break;
...
void DEMO_MainMenu(void) 
{
  ...
  hStatusWin = WM_CreateWindowAsChild(
                                      ...
                                      LCD_GetXSize(),
                                      25,
                                      WM_HBKWIN, WM_CF_SHOW | WM_CF_HASTRANS , _cbStatus, 0);
  
  hIcon = ICONVIEW_CreateEx(0, 24, LCD_GetXSize(), LCD_GetYSize()- 25, WM_HBKWIN, WM_CF_SHOW | WM_CF_HASTRANS ,ICONVIEW_CF_AUTOSCROLLBAR_V ,'0', 112, 96);
